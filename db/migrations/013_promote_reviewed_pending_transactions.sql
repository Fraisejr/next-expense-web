...
begin
  ...
  if candidate.transaction_id is not null then
    ...
    where workspace_id = p_workspace_id
      and id = candidate.transaction_id
  ...
  if existing_transaction.id is not null then
    if existing_transaction.transaction_type = 'transfer' then
      raise exception 'A transfer cannot be approved as a categorized bank transaction.';
    end if;
    created_transaction_id := existing_transaction.id;
    update public.transactions
    set period_id = target_period_id,
        category_id = p_category_id,
        payee_id = linked_payee_id,
    ...
    insert into public.transactions (
      ...
      created_transaction_id, p_workspace_id, candidate.account_id, target_period_id, p_category_id, linked_payee_id,
  ...
  set status = 'approved', category_id = p_category_id, payee_id = linked_payee_id,
      transaction_id = created_transaction_id, decided_at = now()
  where workspace_id = p_workspace_id and id = candidate.id;

  if p_remember_category and linked_payee_id is not null then
