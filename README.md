...
- Timesheet revenue is recognized on the first day of the month after the client work is recorded, matching the invoice month. Client rates are effective-dated against that recognition date so later rate changes do not rewrite earlier revenue. Bank income remains the separate cash-received view.
- Revenue forecasts use client-level weekly hours for the remaining full weeks that can be invoiced within the selected year, less planned vacation weeks. Worked but not yet invoiced hours remain in the forecast until their recognition date. Future daily time entries are not required for forecasting.
