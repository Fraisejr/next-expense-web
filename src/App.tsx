...
function ExpenseApp({ workspace, userName }: { workspace: LoadedWorkspace; userName: string }) {
  ...
  async function editCategory(categoryId: string, changes: Pick<Category, 'name' | 'reportGroup' | 'icon' | 'color' | 'categoryGroupId' | 'hidden'>) {
    ...
    try {
      ...
      const previousCategory = data.categories.find((category) => category.id === categoryId)
      const groupChanged = changes.categoryGroupId !== previousCategory?.categoryGroupId
      const sortOrder = groupChanged
        ? Math.max(-10, ...data.categories.filter((category) => category.id !== categoryId && category.categoryGroupId === changes.categoryGroupId).map((category) => category.sortOrder ?? 0)) + 10
    ...
    <div className="app-shell">
      ...
      {modal && (
        <ModalShell title={modal === 'transaction' ? 'Add transaction' : modal === 'account' ? 'Create account' : modal === 'edit-account' ? 'Edit account' : modal === 'balance-adjustment' ? 'Adjust balance' : modal === 'category' ? 'Create category' : modal === 'edit-category' ? 'Edit category' : modal === 'category-groups' ? 'Manage category groups' : modal === 'monthly-plan' ? `Plan ${monthName.format(viewedMonth)}` : `Connect ${bankTarget?.name ?? 'account'}`} onClose={() => { setModal(null); setAccountTarget(null) }}>
          ...
          {modal === 'edit-category' && selectedCategory && <CategoryDetailsForm category={selectedCategory} categories={data.categories} categoryGroups={data.categoryGroups} onSubmit={editCategory} />}
...
function formatEarnedTimesheetPeriod(today: string, year: number) {
...
function YearlySpendingPlan({ workspaceId, data, defaultCurrency, historyLoading, onSavePlan }: { workspaceId: string; data: AppData; defaultCurrency: string; historyLoading: boolean; onSavePlan: (plan: YearlyFinancialPlan) => Promise<void> }) {
  ...
  const earnedPeriod = formatEarnedTimesheetPeriod(today, currentYear)
  useEffect(() => {
    let cancelled = false
